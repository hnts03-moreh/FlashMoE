...
void gemmMainloop(void* __restrict__ const& workspace,
    ...
    // rmem -> gmem
    flashmoe_blas::copy_fragment<flashmoe_blas::alignment_of<BLAS>::c>(d_frag, gC, accumulator);
}
...
#if !defined(FLASHMOE_PLATFORM_HIP)
// CUDA: full reference + benchmark via MatX
...
void kickStart(const int argc, char** argv) {
    ...
    switch (M) {
    case 1:  driver<arch, 1, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    case 2:  driver<arch, 2, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    case 4:  driver<arch, 4, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    case 8:  driver<arch, 8, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    case 16: driver<arch, 16, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    case 32: driver<arch, 32, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    case 64: driver<arch, 64, bN, bK, (K > bK ? pS : 1), AccumType, Element, ElementC>(M, N, K, rtol, atol, exec); break;
    default:
        if (M >= 128 && M % 128 == 0) {
            if (K > bK) {
...
void driver(const int& M, const int& N, const int& K, const float& rtol, const float& atol, gpuStream_t stream) {
    ...
    constexpr auto min_v = -1.f;
    constexpr auto max_v = 1.f;
    randUniform<Arch>(a, M * K, rd(), min_v, max_v, stream);
...
void kickStart(const int argc, char** argv) {
    ...
    constexpr int bK = sizeof(Element) == 8 ? 32 : 64;
    constexpr auto arch = FLASHMOE_ARCH;
    constexpr int pS = 1; // no async copy on HIP
    constexpr int bN = sizeof(Element) == 8 ? 64 : 64 * (4 / sizeof(Element));
    if (N < bN || N % bN != 0) {
        throw std::runtime_error(std::string("N must be a multiple of ").append(std::to_string(bN)));
    }
    if (K < bK || K % bK != 0) {
        throw std::runtime_error(std::string("K must be a multiple of ").append(std::to_string(bK)));
    }
    gpuSetDevice(0);
    gpuStream_t stream;
    gpuStreamCreate(&stream);
    switch (M) {
    case 1:  driver<arch, 1, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    case 2:  driver<arch, 2, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    case 4:  driver<arch, 4, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    case 8:  driver<arch, 8, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    case 16: driver<arch, 16, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    case 32: driver<arch, 32, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    case 64: driver<arch, 64, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream); break;
    default:
        if (M >= 128 && M % 128 == 0)
            driver<arch, 128, bN, bK, pS, AccumType, Element, ElementC>(M, N, K, rtol, atol, stream);
    }
