...
void driver(const size_t& S, const size_t& E, const size_t& H, const int& k, const float& rtol, const float& atol,
  ...
  nvtx3::scoped_range driverRange{
    std::string("S: ")
  ...
  CHECK_CUDA(cudaMallocAsync(&eCounts, E * sizeof(int), stream));
  CHECK_CUDA(cudaMallocAsync(&eCGuards, E * sizeof(int), stream));
  CHECK_CUDA(cudaMemsetAsync(eCGuards, flashmoe::STALE_AS_BYTE, E * sizeof(int), stream));
  CHECK_CUDA(cudaMallocAsync(&tokens, roundM * K * sizeof(Element), stream));
  CHECK_CUDA(cudaMallocAsync(&gateWeights, K * N * sizeof(Element), stream));
  CHECK_CUDA(cudaMallocAsync(&routing, roundM * N * sizeof(ElementC), stream));
  CHECK_CUDA(cudaMallocManaged(&tokenIds, E * roundEC * sizeof(flashmoe::TPS)));
  if (checkCorrectness) {
    // correctness checking consumes a lot of memory
    CHECK_CUDA(cudaMallocAsync(&routing_ref, M * N * sizeof(ElementC), stream));
    CHECK_CUDA(cudaMallocAsync(&routing_interim, M * N * sizeof(ElementC), stream));
    CHECK_CUDA(cudaMallocManaged(&tokenIds_idx, E * roundEC * sizeof(uint)));
    CHECK_CUDA(cudaMallocManaged(&tokenIds_ref, E * S * sizeof(matx::index_t)));
    CHECK_CUDA(cudaMallocAsync(&topK, S * E * sizeof(matx::index_t), stream));
    CHECK_CUDA(cudaMallocAsync(&eCounts_ref, E * sizeof(matx::index_t), stream));
  }
  if (E > bN) {
    const auto sspSize = S * cute::ceil_div(E, bN) * sizeof(flashmoe::SoftmaxStatePacked);
    CHECK_CUDA(cudaMallocAsync(&rSp, sspSize, stream));
    // only need to reset this once, the kernel internally resets after usage
    CHECK_CUDA(cudaMemsetAsync(rSp, 0, sspSize, stream));
    const auto rtpSize = 2 * S * cute::ceil_div(E, bN) * sizeof(flashmoe::RingTopKPayload);
    CHECK_CUDA(cudaMallocAsync(&rTp, rtpSize, stream));
    // only need to reset this once, the kernel internally resets after usage
    CHECK_CUDA(cudaMemsetAsync(rTp, 0, rtpSize, stream));
  }
  ...
  if (sharedSize > maxSharedMemory) {
    const auto errmsg = std::string("Required shared memory ").append(std::to_string(sharedSize))
                                                              .append(" exceeds hardware limits: ").append(
                                                                std::to_string(maxSharedMemory)).append(
  ...
  // fill a
  randUniform<Arch, /*add jitter*/true>(tokens, M * K, rd(), min_v,max_v, exec.getStream());
  // fill b
  ...
  const GateArgs gArgs{
    ...
    static_cast<int>(S), static_cast<int>(H), static_cast<int>(E), k, static_cast<int>(eCap), static_cast<int>(roundEC),
    rtol, atol
  };
  ...
  if (checkCorrectness) {
    ...
    cudaFreeAsync(routing_interim, stream);
...
void kickStart(const int argc, char** argv) {
  ...
  if (argc > 4) {
    E_max = std::stoi(argv[4]);
  }
  if (argc > 5) {
    k = std::stoi(argv[5]);
  }
  if (argc > 6) {
    checkCorrectness = std::stoi(argv[6]);
  }
  if (argc > 7) {
    rtol = std::stof(argv[7]);
  }
  if (argc > 8) {
    atol = std::stof(argv[8]);
  }
  if (k > E) {
